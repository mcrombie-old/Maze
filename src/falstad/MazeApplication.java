...
public class MazeApplication extends JFrame {
	 ...
	 */
	public MazeApplication(String builder) {
		super() ;
		// scan parameters
		// Case 1: Prim
		...
		// Default case: 
		System.out.println("MazeApplication: unknown parameter value: " + builder + " ignored, operating in default mode.");
		controller = new MazeController() ;
		init() ;
	}
	
	public MazeApplication(String builder, String driver, String fileName) {
		super() ;
		// scan parameters
		// Case 1: Prim
		if ("Prim".equalsIgnoreCase(builder))
		{
			System.out.println("MazeApplication: generating random maze with Prim's algorithm");
			controller = new MazeController(Order.Builder.Prim, driver, fileName) ;
			init() ;
			return ;
		}
		// Case 2: Kruskal
		if ("Kruskal".equalsIgnoreCase(builder))
		{
			System.out.println("MazeApplication: generating random maze with Kruskal's algorithm 3");
		...
		if (f.exists() && f.canRead())
		{
			System.out.println("MazeApplication: loading maze from file: " + fileName);
			controller = new MazeController(fileName) ;
			init();
			return ;
		}
		// Default case: 
	 ...
	 * 
	 * I am adding a couple of extra cases in case th4e user wants to choose which generation alogoirthm or robot
	 *  driver
	 */
	public static void main(String[] args) {
		MazeApplication a ; 
		switch (args.length) {
		case 3 : a = new MazeApplication(args[0], args[1], args[2]);
		break ;
