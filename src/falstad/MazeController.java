...
public class MazeController implements Order {
	...
	protected WallFollower wallFollower;
	protected int driverPathLength = 0;
	protected float driverEnergyConsumed = 0;
	 ...
	 * Constructor that also selects a particular generation method
	 */
	public MazeController(Order.Builder builder, String driver, String fileName)
	{
		super() ;
		robotGame = true; //lets the controller know the robot will be in control and user input is unnecessary
		setBuilder(builder) ;
		panel = new MazePanel() ;
		mazeConfig = new MazeContainer();
		factory = new MazeFactory() ;
	...
	synchronized protected void walk(int dir) {
		if (!checkMove(dir)){
			System.out.println("whasasafdfd");
			return;
			}
	...
	public boolean keyDown(int key) {
		...
		case STATE_GENERATING:
			...
			//if robot chosen to explore maze,
			//robot solves maze using its given driver algorithm
		case STATE_PLAY:
			...
			if (robotGame == false){
			...
			else if (robotGame == true){ //in this case we do automated solver with robot
				
				//System.out.println("In an automated game.");
				
				WallFollower wallFollower = new WallFollower(basicRobot);
				
				//operates driver algorithm on automatic robot
				try {
				...
				driverEnergyConsumed = wallFollower.getEnergyConsumption();
				driverPathLength = wallFollower.getPathLength();
				...
				//manual controls for the sake of my testing
				switch (key){
				case Event.UP: case 'k': case '8':
					// move forward
